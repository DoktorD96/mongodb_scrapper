...
EXPORT.loopfunction = function () {
    ...
    db.each("SELECT `Id`, `Keyword` From `tiktok_tags` WHERE `IsScrapped` = 'false' limit 1", async (error, row) => {
        ...
        //EXPORT.idnow = row.Id;
        db.run('UPDATE `tiktok_tags` SET `Link` = "https://www.tiktok.com/tag/' + row.Keyword + '", `IsScrapped`  = "true" WHERE Id =' + row.Id, async function (err) {
            if (err) {
                if (config.log) {
                    return console.error(err.message);
                }
            }
            // console.log("Scrapping done for " + EXPORT.keywordnow + " with Id " + EXPORT.idnow + ".");
            console.log("https://www.tiktok.com/tag/" + EXPORT.keywordnow + "?is_copy_url=1&is_from_webapp=v1");
            //db.close();
            //await EXPORT.close();
