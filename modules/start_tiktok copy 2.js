...
EXPORT.loopfunction = function () {
    ...
    db.each("SELECT `Id`, `Keyword` From `tiktok_tags` WHERE `IsScrapped` = 'false' ORDER BY  `Popularity` DESC limit 1", async (error, row) => {
        ...
        EXPORT.idnow = row.Id;
        db.run('UPDATE `tiktok_tags` SET `IsScrapped` = "true" WHERE Id =' + row.Id, async function (err) {
            if (err) {
                if (config.log) {
                    return console.error(err.message);
                }
